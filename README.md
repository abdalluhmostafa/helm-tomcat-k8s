...
## Install custom tomcat using bitnami docker image or you custom tomcat image

# 1). Edit values inside ` tomcat-values.yaml ` 

1. Enter you domain name to create ingress and ssl "U must have cert-manager to get free SSL"

2. Number of replica Count

3. Create PVC or not 

4. Image registry , name and tag

5. Enable private registry if u want

6. Custom resoureces CPU/RAM


# 2). Install chart by helm

```
$ git clone https://github.com/abdalluhmostafa/helm-tomcat-k8s

$ helm install tomcatapp . -f tomcat-values.yaml

```

This will let you install current version of chart 

if you want to install latest version of chart using custom values 

` helm install tomcatapp bitnami/tomcat -f tomcat-values.yaml ` 

But I prefer to install with frist way with current version 



# 3). This repo contain example for Dockerfile using tomcat/bitnami image


# 4). Create docker private registry 

If you want create secret for you private registry 

` $ kubectl create secret docker-registry myregistrykey --docker-server=YOUR-DOCKER-REGISTRY-DOMAIN --docker-username=REGISTRY-USER --docker-password=REGISTRY-PASSWORD ` 


---------------------------------------

# Orgain README for tomcat bitnami helm chart 
